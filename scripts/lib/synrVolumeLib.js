...
const abi = require("../../src/config/ABIs.json").contracts.SyndicateCorePool;
const address = require("../../src/config/deployedProduction.json")[1]
  .SyndicateCorePool;
...
// a list of events we want to grab
// defaults to the "important" events for calculating volume
// but can be overridden to get one or all events
function eventTypes(etypes) {
  const contract = new ethers.Contract(address, abi, provider);
  const corePoolEvents = {
    Staked: contract.filters.Staked(),
    Unstaked: contract.filters.Unstaked(),
    YieldClaimed: contract.filters.YieldClaimed(),
    VaultRewardsClaimed: contract.filters.VaultRewardsClaimed(), // unused event
    VaultRewardsReceived: contract.filters.VaultRewardsReceived(), // unused event
    VaultUpdated: contract.filters.VaultUpdated(), // unused event
  };
  let filters = {};
  for (let etype of etypes.split(",")) {
    filters[etype] = corePoolEvents[etype];
  }
  return filters;
}
...
//
// get events for all events on the SYNR contract
...
async function main(opt) {
  ...
  const event_types = await eventTypes(options.types);
  let end = await endPoint();
  console.info("Event types: ", Object.keys(event_types));
  console.time("Events");
